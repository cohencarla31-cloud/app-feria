...
# ==========================================
# 1. CONFIGURACIÓN INICIAL Y CSS ADAPTATIVO
# ==========================================
...
st.markdown("".join(estilos_css) + "".join(script_js), unsafe_allow_html=True)
...
def guardar_efectivo_inicial(link, monto):
    try:
        gc = conectar_google()
        sh = gc.open_by_url(link)
        ws_conf = sh.worksheet("Configuracion")
        records = ws_conf.get_all_values()
        row_idx = -1
        for i, row in enumerate(records):
            if len(row) > 0 and str(row[0]).strip().lower() == "efectivo_inicial":
                row_idx = i + 1
                break
        if row_idx != -1:
            ws_conf.update_cell(row_idx, 2, str(monto))
        else:
            ws_conf.append_row(["efectivo_inicial", str(monto)])
    ...
    try:
        ...
        with st.expander("ℹ️ Pasos para tu Pedido Online", expanded=(st.session_state.web_step == 1)):
            st.markdown(
                ...
                "2️⃣ **Productos:** **PRIMERO ELIGE TODA LA MERCADERÍA A COMPRAR (CON SU PESO)** y luego aprieta el botón de **'IR A MI CARRITO'**.\n"
        ...
        if st.session_state.web_step == 1:
            ...
            for prod_full in productos_ord_web:
                ...
                if desc_p > 0:
                    ...
                    desc_tag = ""
                    precio_str = f"<span style='color:#2e7b32; font-size: 14px; font-weight: bold;'>${pr_fin:,.1f}/{medida_p}</span>"
                
                col_inf, col_bot = st.columns([0.65, 0.35])
                with col_inf:
                    st.markdown(f"<div style='margin-top: 5px;'><b style='font-size: 16px;'>{prod_full}</b>{desc_tag}<br>{precio_str}</div>", unsafe_allow_html=True)
                with col_bot:
            ...
            if st.button("Confirmar y Enviar ➡️", type="primary", use_container_width=True, key="btn_confirm_web"):
                try:
                    ...
                    nombre_mayus = st.session_state.cli_web_nombre.strip().upper()
                    ...
                    try:
                        ...
                        if nombre_mayus.lower() not in nombres_existentes:
...
if st.session_state.rol_logueado == "Admin": 
    ...
    tabs_nombres.append("📥 Reportes Saldos")
...
if st.session_state.rol_logueado in ["Admin", "Cajero", "Vendedor"]:
    with tabs[idx]:
        if 'msg_vendedor' in st.session_state and st.session_state.msg_vendedor:
            ...
            nuevo_modo = st.radio("Acción:", modos_disponibles, horizontal=True, index=modo_idx)
            if nuevo_modo != st.session_state.modo_tomar:
                st.session_state.modo_tomar = nuevo_modo
                st.rerun()
            ...
            if st.session_state.modo_tomar == "🛍️ Venta Local":
                ...
                opcion_nuevo = "➕ Cliente Nuevo (Escribir manual)"
                opciones_cli = [opcion_nuevo] + lista_clientes_base
                
                if not st.session_state.cli_celular:
                    st.session_state.cli_celular = "598"

                def callback_cliente():
                    sel = st.session_state.get(f"sel_cli_loc_{st.session_state.v_rk}", opcion_nuevo)
                    if sel != opcion_nuevo:
                ...
                if not carrito_vend:
                    ...
                    st.warning("⚠️ Revisa que el pedido esté correcto antes de enviarlo a la Caja.")
                    
                    st.markdown("<a href='#inicio_local' style='display: block; text-align: center; background-color: #ffffff; color: #111; padding: 10px; border-radius: 8px; font-weight: bold; font-size: 15px; text-decoration: none; border: 2px solid #111; margin-bottom: 15px;'>⬆️ Volver al catálogo a agregar más</a>", unsafe_allow_html=True)
                    
                    if st.button("🚀 CONFIRMAR Y ENVIAR A CAJA", type="primary", use_container_width=True, key="btn_caja_bottom_loc_conf"):
                        if not st.session_state.cli_nombre:
                            ...
                            det_wsp_local = "\n".join([f"• {r['producto']} ({r['cantidad_txt']})" for r in carrito_vend])
                    ...
                    if not pedidos_web:
                        ...
                        if sel_w != "Seleccionar...":
                            ...
                            for idx_item, it in enumerate(p_sel["items"]):
                                ...
                                sub_r = float(p_real * pr_fin)
                                ahor_iw = float(p_real * (pr_u_base * (desc_u/100)))
                            ...
                            if st.button("⚖️ Confirmar Pesos y Enviar a Caja Web", type="primary", use_container_width=True, key="btn_conf_pesos_web"):
                                ...
                                det_ajustado = "\n".join([f"• {ni['producto']} ({ni['cantidad_txt']})" for ni in nuevos_i])
                                msg_caj_web = f"💳 *CAJA (WEB Ajustado)*\n👤 Cliente: {p_sel['cliente']}\n💰 Total a cobrar: ${tot_real:,.1f}\n📦 Detalle:\n{det_ajustado}"
                                
                                st.session_state.msg_ajuste_web = "✅ ¡Pesos confirmados y enviado a Caja Web!"
                                num_cajero = limpiar_y_formatear_celular(celular_feriante_local)
                                if not num_cajero: num_cajero = "59893343092"
                                st.session_state.link_ajuste_web = f"https://api.whatsapp.com/send?phone={num_cajero}&text={urllib.parse.quote(msg_caj_web)}"
                                st.rerun()
...
if st.session_state.rol_logueado in ["Admin", "Cajero"]:
    with tabs[idx]:
        ...
        if 'msg_web_conf' in st.session_state and st.session_state.msg_web_conf:
            st.success(st.session_state.msg_web_conf)
            st.session_state.msg_web_conf = ""
            ...
            if not p_web: st.info("No hay pedidos web nuevos sin confirmar.")
                ...
                for pw in p_web:
                    with st.container():
                        ...
                        msg_ack = f"👋 Hola {pw['cliente']}, recibimos tu pedido en *{nombre_empresa}* y ya se lo pasamos al equipo para que comience a prepararlo. Te enviaremos a la brevedad el total de tu compra.\n\n📦 *Detalle de tu pedido:*\n{detalle_wsp}\n\n¡Muchas gracias por elegirnos! 💚"
                        ...
                        if st.button("✅ 2. Enviar a Preparar (Confirmar Recepción)", key=f"conf_w_{pw['filas'][0]}", type="primary", use_container_width=True):
                            with st.spinner("Procesando y guardando..."):
                                ...
                                limpiar_cache_ventas()
                                time.sleep(1.5)
                                st.session_state.msg_web_conf = f"✅ Pedido de {pw['cliente']} enviado a 'Ajustar Pedido Web'."
                                st.rerun()
                        st.divider()
        except Exception as e: st.error(f"Error: {e}")
    idx += 1
            ...
            try:
                ...
                if not pedidos_web_caja:
                    ...
                    if sel_w != "Seleccionar...":
                        ...
                        ahorro_web = sum([float(item.get('ahorro', 0)) for item in pw['items']])
                        detalle_wsp_list_f = []
                        for item in pw['items']:
                            detalle_wsp_list_f.append(f"• {item.get('producto', '')} ({item.get('cantidad_txt', '')})")
                        detalle_wsp_final = "\n".join(detalle_wsp_list_f)
    ...
    with tabs[idx]:
        ...
        try:
            ...
            abonos_grales = [
                ...
                or "pago de deuda" in o['detalle'].lower()
            ]
            
            fiados_web = [o for o in todas_o if "web" in o['estado'].lower() and ("pendiente pago" in o['estado'].lower() or "fiado" in o['pago'].lower() or "cuenta" in o['pago'].lower()) and "cancelado" not in o['estado'].lower() and o not in abonos_grales]
            fiados_local = [o for o in todas_o if "web" not in o['estado'].lower() and ("fiado" in o['pago'].lower() or "cuenta" in o['pago'].lower() or "fiado" in o['estado'].lower() or "cuenta" in o['estado'].lower()) and "cancelado" not in o['estado'].lower() and o not in abonos_grales]
            ...
            def consolidar_cuentas(lista_pedidos, abonos):
                ...
                for f_ord in lista_pedidos:
                    ...
                    if cli not in clientes: 
                        clientes[cli] = {"total": 0.0, "pagado": 0.0, "celular": f_ord['celular'], "pedidos": [], "fechas": set(), "tipos": set()}
                    clientes[cli]["total"] += round(f_ord['total'], 1)
                ...
                for ab in abonos:
                    ...
                    if cli_ab in clientes:
                        clientes[cli_ab]["pagado"] += round(abs(ab['total']), 1)
                return clientes
                        ...
                        if st.button("💵 Registrar Pago", type="primary", key=f"btn_reg_l_{cliente_elegido_l}"):
                            if pago_parcial_l > 0:
                                ...
                                if nuevo_saldo_l <= 0.0:
                                    ...
                                    if upds: ws.batch_update(upds)
                                    ws.append_row([ahora.strftime("%d/%m/%Y"), str((ahora + timedelta(seconds=1)).strftime("%H:%M:%S")), st.session_state.usuario_logueado, cliente_elegido_l, "Pago de Deuda Local", 1, -pago_parcial_l, info_cl['celular'], forma_pago_l, "Cobrado", "", 0, "[]"])
                                else:
                                    ...
                                    ws.append_row([ahora.strftime("%d/%m/%Y"), str((ahora + timedelta(seconds=1)).strftime("%H:%M:%S")), st.session_state.usuario_logueado, cliente_elegido_l, "Abono a Cuenta Local", 1, -pago_parcial_l, info_cl['celular'], forma_pago_l, "Abono Local", "", 0, "[]"])
        ...
        try:
            ...
            if not ent_pendientes: st.info("No hay entregas pendientes por llevar.")
                ...
                if sel_ent != "Seleccionar...":
                    ...
                    st.link_button("📲 1. Avisar 'Va en Camino'", link_wsp, type="primary", use_container_width=True)
...
if st.session_state.rol_logueado == "Admin":
    with tabs[idx]:
        ...
        with col_f1:
            fecha_arqueo = st.date_input("📅 Fecha del Arqueo:", value=datetime.now(TZ_UY).date())
        fecha_str = fecha_arqueo.strftime("%d/%m/%Y")
        
        try:
            ordenes_hoy = [p for p in agrupar_pedidos(ventas_data_global) if p['fecha'] == fecha_str]
            
            tot_efectivo = 0.0
            tot_digital = 0.0
            tot_fiado = 0.0
            
            for p in ordenes_hoy:
                est = p['estado'].lower()
                ...
                if not is_payment_row and (pago_str in ["a cuenta", "fiado", "pendiente pago", "envío cuenta"] or "pendiente" in est):
                    tot_fiado += monto
                else:
                    if "efectivo" in pago_str:
                        tot_efectivo += monto
                    else:
                        tot_digital += monto
                        
            tot_recaudado_hoy = tot_efectivo + tot_digital
            ...
            st.info(f"📝 **Ventas a Cuenta (Fiado) de este día:** ${tot_fiado:,.1f} *(Mercadería entregada pero no cobrada)*")
            
            st.divider()
            st.markdown("#### 🔒 Arqueo de Caja Físico (Billetes)")
        ...
        try:
            ...
            for p in ordenes_admin:
                ...
                if "cancelado" not in est and "caja" not in est and "devuelto" not in est:
                    ...
                    if is_payment_row:
                        concepto = f"Cobro de Deuda en {pago_bruto}{origen}"
                    elif "pendiente" in est or pago_bruto == "A Cuenta / Fiado":
                        concepto = f"Venta Fiada / No Cobrada{origen}"
                    else:
                    ...
                    vendedores_resumen[key_vend] = vendedores_resumen.get(key_vend, 0.0) + monto
                    
                if "cancelado" not in est and p['total'] > 0:
...
if st.session_state.rol_logueado == "Admin":
    with tabs[idx]:
        ...
        try:
            ...
            fiados_activos = [o for o in todas_w if ("fiado" in o['pago'].lower() or "fiado" in o['estado'].lower() or "cuenta" in o['pago'].lower() or "cuenta" in o['estado'].lower() or "pendiente pago" in o['estado'].lower()) and "cancelado" not in o['estado'].lower() and o not in abonos_grales]
            ...
            for ab in abonos_grales:
                cli_ab = str(ab['cliente']).strip().title()
                if cli_ab in resumen_saldos:
                    resumen_saldos[cli_ab]["Pagado"] += round(abs(ab['total']), 2)
